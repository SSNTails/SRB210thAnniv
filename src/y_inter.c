...
void Y_IntermissionDrawer(void)
{
				...
				else
				{
					...
					else if (inttype == int_timerace)
					{
						...
						else if (players[data.match.num[i]].lives <= 0)
						{
							...
							snprintf(time, sizeof time, "%lu:%02lu.%02lu", data.match.scores[i]/(60*TICRATE),
									data.match.scores[i]/TICRATE % 60, data.match.scores[i]%TICRATE);
	...
	else if (inttype == int_race)
	{
		...
		for (i = 0; i < data.race.numplayersshown; i++)
		{
			...
			// draw score
			snprintf(name, sizeof name, "%lu", data.race.scores[i]);
...
static void Y_CalculateMatchWinners(void)
{
	...
	for (j = 0; j < MAXPLAYERS; j++)
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
		{
			...
			if (gametype == GT_TAG)
			{
				if (players[i].tagcount >= data.match.scores[data.match.numplayers] && completed[i] == false)
				{
					data.match.scores[data.match.numplayers] = players[i].tagcount;
