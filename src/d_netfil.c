...
byte *PutFileNeeded(void)
{
	...
	for (i = 0; i < numwadfiles; i++)
	{
		...
		bytesused += (strlen(wadfilename) + 22);
...
void SendFile(int node, char *filename, char fileid)
{
	...
	nameonly(strncpy(p->filename, filename, MAX_WADPATH));

	// check first in wads loaded the majority of case
	for (i = 0; wadfiles[i]; i++)
	{
		nameonly(strncpy(wadfilename, wadfiles[i]->filename, MAX_WADPATH));
		if (!stricmp(wadfilename, p->filename))
		{
			// copy filename with full path
			strncpy(p->filename, wadfiles[i]->filename, MAX_WADPATH);
			break;
		}
	}
...
void Got_Filetxpak(void)
{
	...
	if (fileneeded[filenum].status == FS_DOWNLOADING)
	{
		...
		if (filetime == 0)
		{
			...
			V_DrawCenteredString(BASEVIDWIDTH/2, (BASEVIDHEIGHT/2)+12, 0, va("\r%s %luK/%luK %.1fK/s\n",fileneeded[filenum].filename,
		                                   fileneeded[filenum].currentsize>>10,
		                                   fileneeded[filenum].totalsize>>10,
		                                   ((double)getbps)/1024));
		}
