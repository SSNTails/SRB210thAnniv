...
static inline VOID ShowEndTxt(HANDLE co)
{
	...
	for (i = 1; i <= 80*25; i++) // print 80x25 text and deal with the attributes too
	{
		...
		let = (char)(*text & 0xff); // text senond
...
int I_StartupSystem(void)
{
	...
	hr = DirectInputCreateA(myInstance, DIRECTINPUT_VERSION, &lpDI, NULL);
	...
	else
	{
		...
		hr = DirectInputCreateA(myInstance, DXVERSION_NTCOMPATIBLE, &lpDI, NULL);
