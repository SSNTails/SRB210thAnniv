...
boolean I_SetSongSpeed(float speed)
{
	...
	if (fmus && FSOUND_IsPlaying(fsoundchannel))
	{
		if (!FSOUND_SetFrequency(fsoundchannel,(int)(speed*fsoundfreq)))
		{
			if (devparm)
	...
	else if (mod && FMUSIC_IsPlaying(mod))
	{
		if (FMUSIC_SetMasterSpeed(mod, speed))
		{
			if (devparm)
...
#ifdef FMODMEMORY
	mod = FMUSIC_LoadSongEx(data, 0, (int)len, ((looping) ? (FSOUND_LOOP_NORMAL) : (0))|FSOUND_LOADMEMORY, NULL, 0);
#else
...
//		}
	...
	{
#ifdef FMODMEMORY
		fmus = FSOUND_Stream_Open(data, ((looping) ? (FSOUND_LOOP_NORMAL) : (0))|FSOUND_LOADMEMORY, 0, (int)len);
#else
		fmus = FSOUND_Stream_Open("fmod.tmp", ((looping) ? (FSOUND_LOOP_NORMAL) : (0)), 0, len);
#endif
		if (fmus == NULL)
		{
