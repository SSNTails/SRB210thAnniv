...
boolean P_TeleportMove(mobj_t *thing, fixed_t x, fixed_t y, fixed_t z)
{
	...
	P_CheckPosition(thing, thing->x, thing->y, thing->x, thing->y);
...
static void add_spechit(line_t *ld)
{
	...
	if (numspechit >= spechit_max)
	{
		...
		if (!spechit)
			I_Error("Out of memory in add_spechit()");
	}
...
static boolean PIT_CheckThing(mobj_t *thing)
{
	...
	{
		if ((gametype == GT_MATCH && (!cv_teamplay.value
			|| (cv_teamplay.value == 1 && tmthing->player->skincolor != thing->player->skincolor)
			|| (cv_teamplay.value == 2 && tmthing->player->skin != thing->player->skin)))
			|| (gametype == GT_CTF && tmthing->player->ctfteam != thing->player->ctfteam)
			|| (gametype == GT_RACE))
		{
	...
	if (cv_tailspickup.value)
	{
		if (tmthing->player && thing->player)
		{
					...
					|| (tmthing->player->charability == 1 && (tmthing->state == &states[S_PLAY_SPC1] || tmthing->state == &states[S_PLAY_SPC2] || tmthing->state == &states[S_PLAY_SPC3] || tmthing->state == &states[S_PLAY_SPC4])))
			{
				...
				if ((!(tmthing->eflags & MF_VERTICALFLIP) && (tmthing->z <= thing->z + thing->height + FRACUNIT)
					&& tmthing->z > thing->z + thing->height*2/3
					&& thing->momz <= 0)
					|| ((tmthing->eflags & MF_VERTICALFLIP) && (tmthing->z + tmthing->height >= thing->z - FRACUNIT)
	...
	if (thing->player)
	{
		...
		if (gametype == GT_TAG && tmthing->player && (((thing->z <= tmthing->z + tmthing->height) && (thing->z + thing->height >= tmthing->z)) || (tmthing->z == thing->z + thing->height + FRACUNIT)))
		{
			if (thing->player->tagit < 298*TICRATE && thing->player->tagit > 0 && !(tmthing->player->powers[pw_flashing] || tmthing->player->tagzone || tmthing->player->powers[pw_invulnerability]))
			{
				...
				if ((thing->player->pflags & PF_JUMPED) && thing->momz < 0) // Bounce tagger off target
			...
			else if (tmthing->player->tagit < 298*TICRATE && tmthing->player->tagit > 0 && !(thing->player->powers[pw_flashing] || thing->player->tagzone || thing->player->powers[pw_invulnerability]))
			{
				...
				if ((tmthing->player->pflags & PF_JUMPED) && tmthing->momz < 0) // Bounce tagger off target
					tmthing->momz = -tmthing->momz;
			}
		}

		if (thing->eflags & MF_VERTICALFLIP && tmthing->eflags & MF_VERTICALFLIP)
		{
			...
			if (thing->z + thing->height <= tmthing->z + tmthing->height && !(thing->state == &states[thing->info->painstate])) // Stuff where da player don't gotta move
			{
				...
				{
					case MT_FAN: // fan
						if (thing->z + thing->height >= tmthing->z + tmthing->height - (tmthing->health << FRACBITS) && thing->momz > -tmthing->info->speed)
						{
	...
	if (tmthing->player) // Is the moving/interacting object the player?
	{
		...
		if ((thing->z <= tmthing->z + tmthing->height
			...
			|| ((!(tmthing->eflags & MF_VERTICALFLIP) && tmthing->z == thing->z + thing->height + FRACUNIT)
				|| ((tmthing->eflags & MF_VERTICALFLIP) && tmthing->z + tmthing->height == thing->z - FRACUNIT)))
		{
			if (thing->flags & MF_SPRING)
			{
				...
				&& ((tmthing->player->pflags & PF_JUMPED) || (tmthing->player->pflags & PF_SPINNING) || maptol & TOL_ADVENTURE))
			{
				// Going down? Then bounce back up.
				if (tmthing->eflags & MF_VERTICALFLIP)
				{
					if (tmthing->momz > 0)
						tmthing->momz = -tmthing->momz;
				}
...
//
boolean P_CheckPosition(mobj_t *thing, fixed_t oldx, fixed_t oldy, fixed_t x, fixed_t y)
{
	...
	if (oldx != x || oldy != y)
	{
		tobbox[BOXTOP] = oldy + tmthing->radius;
		tobbox[BOXBOTTOM] = oldy - tmthing->radius;
		tobbox[BOXRIGHT] = oldx + tmthing->radius;
		tobbox[BOXLEFT] = oldx - tmthing->radius;
	}
	else
	{
		tobbox[BOXTOP] = tmbbox[BOXTOP];
		tobbox[BOXBOTTOM] = tmbbox[BOXBOTTOM];
		tobbox[BOXRIGHT] = tmbbox[BOXRIGHT];
		tobbox[BOXLEFT] = tmbbox[BOXLEFT];
	}
	...
	if (newsubsec->sector->ffloors)
	{
		...
		for (rover = newsubsec->sector->ffloors; rover; rover = rover->next)
		{
			...
			{
				if (!(thing->player && !(thing->player->pflags & PF_NIGHTSMODE) && (((thing->player->charflags & SF_RUNONWATER) || thing->player->powers[pw_super]) && thing->ceilingz-*rover->topheight >= thing->height)
...
/*					&& thing->ceilingz - *rover->topheight >= thing->height*/
	...
	if (newsubsec->polyList)
	{
		...
		while(po)
		{
			if (!(po->flags & POF_CLIPPLANES) || !P_PointInsidePolyobj(po, x, y))
			{
			...
			delta1 = thing->z - (polysec->floorheight + ((polysec->ceilingheight - polysec->floorheight)/2));
			delta2 = thingtop - (polysec->floorheight + ((polysec->ceilingheight - polysec->floorheight)/2));

			if (polysec->ceilingheight > tmfloorz && abs(delta1) < abs(delta2))
				tmfloorz = tmdropoffz = polysec->ceilingheight;

			if (polysec->floorheight < tmceilingz && abs(delta1) >= abs(delta2))
				tmceilingz = polysec->floorheight;
...
static boolean P_CheckCameraPosition(fixed_t x, fixed_t y, camera_t *thiscam)
{
	...
	if (newsubsec->polyList)
	{
		...
		while(po)
		{
			...
			delta1 = thiscam->z - (polysec->floorheight + ((polysec->ceilingheight - polysec->floorheight)/2));
			delta2 = thingtop - (polysec->floorheight + ((polysec->ceilingheight - polysec->floorheight)/2));
...
boolean P_TryMove(mobj_t *thing, fixed_t x, fixed_t y, boolean allowdropoff)
{
	...
	if (!P_CheckPosition(thing, thing->x, thing->y, x, y))
	{
	...
	if (!(thing->flags & MF_NOCLIP))
	{
		...
		if (thing->player)
		{
			...
			if (thing->state == &states[S_PLAY_PLG1]
				&& ((!(thing->eflags & MF_VERTICALFLIP) && thing->momz > FRACUNIT)
				|| ((thing->eflags & MF_VERTICALFLIP && thing->momz < -FRACUNIT))))
				maxstep = 0;
		}
		...
		if (thing->eflags & MF_VERTICALFLIP)
		{
			if (thing->z < tmfloorz)
			{
				CheckMissileImpact(thing);
		...
		if (thing->player && GETSECSPECIAL(thing->player->specialsector, 1) == 13)
			maxstep <<= 1;

		// Ramp test
		if (thing->player && (GETSECSPECIAL(thing->player->specialsector, 1) != 14
			&& GETSECSPECIAL(R_PointInSubsector(x, y)->sector->special, 1) != 14) && thing->z == thing->floorz
...
static boolean PTR_AimTraverse(intercept_t *in)
{
	...
	if (aim_nofriends
		&& th->player
		&& ((gametype == GT_CTF && th->player->ctfteam == aim_nofriends)
		|| (gametype == GT_MATCH && cv_teamplay.value == 1 && th->player->skincolor == aim_nofriends-1)
		|| (gametype == GT_MATCH && cv_teamplay.value == 2 && th->player->skin == aim_nofriends-1)
		|| aim_nofriends == -1))
		return true;
...
fixed_t P_AimLineAttack(mobj_t *t1, angle_t angle, fixed_t distance)
{
	...
	if (t1->player)
	{
		...
		if (gametype == GT_CTF) // Team
			aim_nofriends = t1->player->ctfteam;
		else if (gametype == GT_MATCH && cv_teamplay.value == 1) // Color
			aim_nofriends = t1->player->skincolor + 1;
		else if (gametype == GT_MATCH && cv_teamplay.value == 2) // Skin
			aim_nofriends = t1->player->skin + 1;
		...
		for (i = 0; i < sector->numattached; i++)
		{
			...
			do
			{
				...
				if (!n->visited)
				{
					...
					if (!(n->m_thing->flags & MF_NOBLOCKMAP))
					{
						if (!PIT_ChangeSector(n->m_thing, true))
							goto donewiththisparttoo;
					}
					break;
				}
			} while (n);
	...
	do
	{
		for (n = sector->touching_thinglist; n; n = n->m_snext) // go through list
			if (!n->visited) // unprocessed thing found
			{
				...
				if (!(n->m_thing->flags & MF_NOBLOCKMAP)) //jff 4/7/98 don't do these
				{
					if (!PIT_ChangeSector(n->m_thing, true)) // process it
						return nofit;
				}
				break; // exit and start over
			}
