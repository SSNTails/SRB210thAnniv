...
static void R_AddLine(seg_t *line)
{
	...
	if (line->linedef->flags & ML_BLOCKING) // Don't even draw 1-sided lines
		return;
	else if (
#ifdef POLYOBJECTS
		!line->polyseg &&
...
static size_t numpolys;        // number of polyobjects in current subsector
static size_t num_po_ptrs;     // number of polyobject pointers allocated
...
{
   const polyobj_t *po1 = *(polyobj_t * const *)p1;
   const polyobj_t *po2 = *(polyobj_t * const *)p2;
...
void R_Prep3DFloors(sector_t *sector)
{
	...
	if (count != sector->numlights)
	{
		...
		sector->lightlist = Z_Malloc(sizeof (*sector->lightlist) * count, PU_LEVEL, NULL);
