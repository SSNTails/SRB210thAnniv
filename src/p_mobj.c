...
boolean P_SetPlayerMobjState(mobj_t *mobj, statenum_t state)
{
	...
	if (mobj->player)
	{
		...
		else if (mobj->state == &states[mobj->info->painstate] && mobj->player->powers[pw_flashing] == flashingtics)
	...
	for (i = 0, j = 0; i < numtospawn; i++)
	{
		...
		while (true)
		{
			...
			if (leveltime < TICRATE) // Start of map
				spawnpoints[j]->threshold = 60*TICRATE + P_Random() * (TICRATE/7);
			else
				spawnpoints[j]->threshold = P_Random() * (TICRATE/7);
...
void P_XYMovement(mobj_t *mo)
{
	...
	if ((mo->type == MT_FLINGRING
		...
		&& ((mo->z <= mo->subsector->sector->floorheight && !(mo->eflags & MF_VERTICALFLIP))
		|| (mo->z + mo->height >= mo->subsector->sector->ceilingheight && (mo->eflags & MF_VERTICALFLIP)))
...
bustupdone:
	...
	do
	{
		...
		if (!P_TryMove(mo, ptryx, ptryy, true) && !tmsprung)
		{
			...
			if ((mo->flags & MF_BOUNCE) && !(mo->type == MT_THROWNBOUNCE && mo->threshold > 5))
			{
				P_BounceMove(mo);
				xmove = ymove = 0;
	...
	if (((!(mo->eflags & MF_VERTICALFLIP) && mo->z > mo->floorz) || (mo->eflags & MF_VERTICALFLIP && mo->z+mo->height < mo->ceilingz)) && !(mo->flags2 & MF2_ONMOBJ)
	...
	if (((!(mo->eflags & MF_VERTICALFLIP) && mo->z > mo->floorz) || (mo->eflags & MF_VERTICALFLIP && mo->z+mo->height < mo->ceilingz)) && mo->type != MT_CRAWLACOMMANDER && mo->type != MT_EGGMOBILE && mo->type != MT_EGGMOBILE2
		&& !(mo->player && (mo->player->pflags & PF_SLIDING)))
		return; // no friction when airborne
...
static void P_ZMovement(mobj_t *mo)
{
	...
	if (mo->z + mo->height > mo->ceilingz && !(mo->flags & MF_NOCLIPHEIGHT))
	{
		...
		{
			// Flingrings bounce
			if ((mo->eflags & MF_VERTICALFLIP) && (mo->type == MT_FLINGRING
				|| mo->type == MT_FLINGCOIN
				|| mo->type == MT_BOUNCERING
				|| mo->type == MT_AUTOMATICRING
				|| mo->type == MT_INFINITYRING
...
static void P_PlayerZMovement(mobj_t *mo)
{
	...
	if (mo->subsector->sector->ffloors)
	{
		...
		for (rover = mo->subsector->sector->ffloors; rover; rover = rover->next)
		{
			...
			if ((!(rover->flags & FF_SOLID || rover->flags & FF_QUICKSAND) && !(mo->player && !(mo->player->pflags & PF_NIGHTSMODE) && !mo->player->homing && (((mo->player->charflags & SF_RUNONWATER) || mo->player->powers[pw_super]) && mo->ceilingz-*rover->topheight >= mo->height) && (rover->flags & FF_SWIMMABLE) && !(mo->player->pflags & PF_SPINNING) && mo->player->speed > mo->player->runspeed && /*mo->ceilingz - *rover->topheight >= mo->height && */mo->z < *rover->topheight + 30*FRACUNIT && mo->z > *rover->topheight - 30*FRACUNIT)))
				continue;
	...
	if ((mo->eflags & MF_VERTICALFLIP && mo->z + mo->height > mo->ceilingz)
		|| (!(mo->eflags & MF_VERTICALFLIP) && mo->z < mo->floorz))
	{
		if (mo->eflags & MF_VERTICALFLIP)
		{
			mo->player->viewheight += mo->ceilingz - mo->z+mo->height;
...
/*	if (mo->pmomz && mo->z > mo->floorz && !(mo->player->pflags & PF_JUMPED))
	...
	if (P_IsObjectOnGround(mo))
	{
		...
		if ((!(mo->eflags & MF_VERTICALFLIP) && mo->momz < 0) || (mo->eflags & MF_VERTICALFLIP && mo->momz > 0)) // falling
		{
			...
			else if ((maptol & TOL_ADVENTURE) && mo->momz < -2*FRACUNIT && mo->info->activesound && !tmfloorthing)
				S_StartSound(mo, mo->info->activesound);
...
nightsdone:
	...
	if ((mo->eflags & MF_VERTICALFLIP && mo->z < mo->floorz) || (!(mo->eflags & MF_VERTICALFLIP) && mo->z + mo->height > mo->ceilingz))
	{
		if (mo->player && (mo->player->pflags & PF_NIGHTSMODE))
		{
			if (mo->player->flyangle < 90 || mo->player->flyangle >= 270)
		...
		if ((mo->eflags & MF_VERTICALFLIP && mo->momz < 0)
			|| (!(mo->eflags & MF_VERTICALFLIP) && mo->momz > 0))
		{
			msecnode_t *node;
...
void P_MobjCheckWater(mobj_t *mobj)
{
	...
	if (sector->ffloors) // 3D water
	{
		...
		for (rover = sector->ffloors; rover; rover = rover->next)
		{
			...
			if (mobj->eflags & MF_VERTICALFLIP)
			{
			...
			if (((mobj->eflags & MF_VERTICALFLIP) && mobj->z < *rover->bottomheight)
				|| (!(mobj->eflags & MF_VERTICALFLIP) && mobj->z + mobj->info->height > *rover->topheight))
			...
			if (((mobj->eflags & MF_VERTICALFLIP) && mobj->z + (mobj->info->height/2) > *rover->bottomheight)
				|| (!(mobj->eflags & MF_VERTICALFLIP) && mobj->z + (mobj->info->height/2) < *rover->topheight))
			{
		...
		&& ((mobj->eflags & MF_UNDERWATER) != wasinwater))
	{
		...
		if (!(mobj->eflags & MF_VERTICALFLIP) && mobj->watertop-mobj->floorz <= mobj->info->height>>1)
			return;

		if ((mobj->eflags & MF_VERTICALFLIP) && mobj->ceilingz-mobj->waterbottom <= mobj->info->height>>1)
			return;

		if (wasinwater && ((mobj->eflags & MF_VERTICALFLIP && mobj->momz < 0) || (!(mobj->eflags & MF_VERTICALFLIP) && mobj->momz > 0)))
		{
			...
			if (mobj->player)
				P_SetObjectMomZ(mobj, FixedDiv(mobj->momz, FixedDiv(100*FRACUNIT,mobj->player->jumpfactor*FRACUNIT)), false);
		}

		if ((mobj->eflags & MF_VERTICALFLIP && mobj->momz > 0) || (!(mobj->eflags & MF_VERTICALFLIP) && mobj->momz < 0))
		{
			if ((mobj->eflags & MF_VERTICALFLIP && mobj->z+(mobj->info->height>>1)-mobj->momz <= mobj->waterbottom)
				|| (!(mobj->eflags & MF_VERTICALFLIP) && mobj->z+(mobj->info->height>>1)-mobj->momz >= mobj->watertop))
			{
				// Spawn a splash
				if (mobj->eflags & MF_VERTICALFLIP)
			...
			// skipping stone!
			if (mobj->player && (mobj->player->charflags & SF_WATERSKIP) && !(mobj->player->pflags & PF_JUMPED) && mobj->player->speed/2 > abs(mobj->momz>>FRACBITS)
				&& (mobj->player->pflags & PF_SPINNING) && mobj->z + mobj->info->height - mobj->momz > mobj->watertop)
			{
				mobj->momz = -mobj->momz/2;

				if (!(mobj->eflags & MF_VERTICALFLIP) && mobj->momz > 6*FRACUNIT)
					mobj->momz = 6*FRACUNIT;
				else if (mobj->eflags & MF_VERTICALFLIP && mobj->momz < -6*FRACUNIT)
					mobj->momz = -6*FRACUNIT;
			}

		}
		else if ((mobj->eflags & MF_VERTICALFLIP && mobj->momz < 0) || (!(mobj->eflags & MF_VERTICALFLIP) && mobj->momz > 0))
		{
			if ((mobj->eflags & MF_VERTICALFLIP && mobj->z+(mobj->info->height>>1)-mobj->momz > mobj->waterbottom)
				|| (!(mobj->eflags & MF_VERTICALFLIP) && mobj->z+(mobj->info->height>>1)-mobj->momz < mobj->watertop))
			{
		...
		for (i = 0; i < bubblecount; i++)
		{
			...
			if (bubble)
			{
				if ((mobj->eflags & MF_VERTICALFLIP && mobj->momz > 0)
					|| (!(mobj->eflags & MF_VERTICALFLIP) && mobj->momz < 0))
...
static void P_PlayerMobjThinker(mobj_t *mobj)
{
	...
	if (mobj->tracer && mobj->tracer->type == MT_TUBEWAYPOINT)
	{
		...
		P_CheckPosition(mobj, mobj->x, mobj->y, mobj->x, mobj->y);
		goto animonly;
	}
	...
	for (node = mobj->touching_sectorlist; node; node = node->m_snext)
	{
		...
		for (rover = node->m_sector->ffloors; rover; rover = rover->next)
		{
			...
			if ((rover->flags & FF_CRUMBLE)
				&& ((*rover->topheight == mobj->z && !(mobj->eflags & MF_VERTICALFLIP))
				|| (*rover->bottomheight == mobj->z + mobj->height && (mobj->eflags & MF_VERTICALFLIP)))) // You nut.
	...
	if (!(mobj->eflags & MF_ONGROUND) || mobj->momz
		|| ((mobj->eflags & MF_VERTICALFLIP) && mobj->z + mobj->height != mobj->ceilingz)
		|| (!(mobj->eflags & MF_VERTICALFLIP) && mobj->z != mobj->floorz))
	{
		mobj_t *onmo;
		...
		{
			P_PlayerZMovement(mobj);
			P_CheckPosition(mobj, mobj->x, mobj->y, mobj->x, mobj->y); // Need this to pick up objects!
...
//	thinker_t *th;
	...
	(void)sector; // Advanced precip still doesn't work =(=(
	...
	if (mobj->reactiontime && mobj->health > mobj->info->damage) // Shock mode
	{
		...
		if (leveltime % 2*TICRATE == 0)
		{
			...
			for (i = 0; i < MAXPLAYERS; i++)
			{
				...
				if (players[i].mo->eflags & MF_UNDERWATER)
...
void P_SpawnParaloop(fixed_t x, fixed_t y, fixed_t z, fixed_t radius, int number, mobjtype_t type, angle_t rotangle, boolean spawncenter)
{
	mobj_t *mobj;
	int i;
	TVector v;
	...
	if (mobj->flags & MF_SCENERY)
	{
		switch (mobj->type)
		{
			...
			case MT_DROWNNUMBERS:
				...
				if (mobj->target->eflags & MF_VERTICALFLIP)
			...
			switch (mobj->type)
			{
				...
				case MT_GRAVITYBOX:
					...
					if (mobj->flags & MF_AMBUSH)
					{
						...
						if (!(cv_1up.value || cv_bombshield.value || cv_forceshield.value
							|| cv_ringshield.value || cv_watershield.value || cv_jumpshield.value
							|| cv_invincibility.value || cv_supersneakers.value
...
consvar_t cv_itemrespawntime = {"respawnitemtime", "30", CV_NETVAR, respawnitemtime_cons_t, NULL, 0, NULL, NULL, 0, 0, NULL};
...
void P_SpawnPlayer(mapthing_t *mthing, int playernum)
{
	...
	if (p->mo->eflags & MF_VERTICALFLIP)
...
void P_SpawnMapThing(mapthing_t *mthing)
{
	...
	if (mthing->type != 1701 && mthing->type != 1702 && mthing->type != 1700
	...
	if (i == NUMMOBJTYPES)
	{
		if (mthing->type != 32000 && mthing->type != 32001) // 3D mode start and Control Sector Position Hint from DB
	...
	if (gametype == GT_RACE)
	{
		...
		{
			if (i == MT_BLUETV && !cv_forceshield.value) return;
			if (i == MT_YELLOWTV && !cv_ringshield.value) return;
			if (i == MT_GREENTV && !cv_watershield.value) return;
		...
		)
	{
		...
		else // cv_matchboxes.value == 0, Normal
		{
			...
			boxes[0].type = MT_BLUETV;
			boxes[0].consvar = &cv_forceshield;
			boxes[1].type = MT_YELLOWTV;
			boxes[1].consvar = &cv_ringshield;
			boxes[2].type = MT_GREENTV;
			boxes[2].consvar = &cv_watershield;
			boxes[3].type = MT_BLACKTV;
			boxes[3].consvar = &cv_bombshield;
			boxes[4].type = MT_WHITETV;
			boxes[4].consvar = &cv_jumpshield;
			boxes[5].type = MT_SNEAKERTV;
			boxes[5].consvar = &cv_supersneakers;
			boxes[6].type = MT_SUPERRINGBOX;
			boxes[6].consvar = &cv_superring;
			boxes[7].type = MT_INV;
			boxes[7].consvar = &cv_invincibility;
			boxes[8].type = MT_EGGMANBOX;
			boxes[8].consvar = &cv_eggmanbox;
			boxes[9].type = MT_MIXUPBOX;
			boxes[9].consvar = &cv_teleporters;
			boxes[10].type = MT_PRUP;
			boxes[10].consvar = NULL; // no meaning in CTF/tag/chaos/match
			...
			for (g = 0; g < 11; g++)
			{
				...
				while(true)
				{
					// All boxes are turned off, argh.
					if (counted >= 11)
						return;

					// no meaning in CTF/tag/chaos/match
					if (!boxes[trybox].consvar)
					{
						// Switch to another box and try that.
						counted++;
						trybox++;
						trybox %= 11;
						continue;
					...
					if (boxes[trybox].consvar->value) // This box is enabled! Yay!
					{
						i = boxes[trybox].type;
						goto thisboxisokay;
					}
...
thisboxisokay:
	...
	{
		fixed_t length, speed, xspeed, zspeed, startangle, maxspeed;
...
*/
		length = abs(lines[line].dx >> FRACBITS);
		speed = abs(lines[line].dy >> FRACBITS);
		xspeed = sides[lines[line].sidenum[0]].textureoffset >> FRACBITS;
		zspeed = sides[lines[line].sidenum[0]].rowoffset >> FRACBITS;
		startangle = lines[line].frontsector->floorheight >> FRACBITS;
		maxspeed = lines[line].frontsector->ceilingheight >> 8;
		...
		mobj->lastlook = speed << 4;
		mobj->movecount = mobj->lastlook;
		mobj->health = (zspeed*FINEANGLES/360) + (startangle*FINEANGLES/360);
		mobj->threshold = (xspeed*FINEANGLES/360) + (startangle*FINEANGLES/360);
		mobj->movefactor = mobj->threshold;
		mobj->friction = maxspeed;
		...
		else
		{
			...
			if (mobj->type == MT_SWINGMACEPOINT)
				spawnee->movecount = startangle*(FINEANGLES/360);
			else
				spawnee->movecount = 0;

			spawnee->threshold = startangle*(FINEANGLES/360);
			spawnee->reactiontime = length+1;
		}
...
void P_SpawnHoopsAndRings(mapthing_t *mthing)
{
	...
	else
	{
		...
		if (mthing->type == 300) // Your basic ring.
		{
			unsigned int bit;
		...
		else if (mthing->type == 1800) // Your basic coin.
		{
			unsigned int bit;
...
//
mobj_t *P_SPMAngle(mobj_t *source, mobjtype_t type, angle_t angle, boolean noaiming, int flags2)
{
	mobj_t *th;
	angle_t an;
