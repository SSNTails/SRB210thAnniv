...
static void CL_LoadReceivedSavegame(void)
{
	...
	if (P_LoadNetGame())
	{
		...
		CONS_Printf("Map is now \"%s: %s", G_BuildMapName(gamemap), mapheaderinfo[gamemap-1].lvlttl);
...
static void CL_ConnectToServer(boolean viams)
{
	...
	if (i != -1)
	{
		...
		if (gametypestr)
			 CONS_Printf("Gametype: %s\n", gametypestr);
		CONS_Printf("Version: %d.%.2d.%u\n", serverlist[i].info.version/100,
...
void D_SaveBan(void)
{
	...
	if (!f)
	{
		CONS_Printf("Could not save ban list into ban.txt\n");
		return;
	}
...
static void Ban_Load_File(boolean warning)
{
	...
	{
		if (warning)
			CONS_Printf("Could not open ban.txt for ban list\n");
		return;
	}
...
static void Command_Nodes(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		if (playeringame[i])
		{
			CONS_Printf("%.2d: %*s", i, maxlen, player_names[i]);
...
consvar_t cv_maxsend = {"maxsend", "512", CV_SAVE, maxsend_cons_t, NULL, 0, NULL, NULL, 0, 0, NULL};
...
static void Got_AddPlayer(byte **p, int playernum)
{
	...
	else if (server && cv_showjoinaddress.value)
	{
		...
		if (I_GetNodeAddress && (address = I_GetNodeAddress(node)) != NULL)
			CONS_Printf(" Player Address is %s\n", address);
	}
		...
		if (!nodeingame[node])
		{
			...
			switch (netbuffer->packettype)
			{
				...
				case PT_SERVERCFG: // positive response of client join request
				{
					...
					CONS_Printf("Join accepted, wait next level change...\n");
