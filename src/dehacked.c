		...
		if (myfgets(s, sizeof (s), f))
		{
			...
			*(tmp-1) = '\0';
			...
			else if (!strcmp(word, "SCRIPTNAME"))
			{
				...
				strncpy(mapheaderinfo[num-1].scriptname, word2, 192);
			}
			...
			else if (!strcmp(word, "RUNSOC"))
			{
				...
				strncpy(mapheaderinfo[num-1].runsoc, word2, 64);
			}
			else
			...
			if (word2)
			{
				...
				if (!strcmp(word, "THING"))
				{
					if (i < NUMMOBJTYPES && i >= 0)
						...
						deh_warning("Thing %d doesn't exist", i);
...
/*				else if (!strcmp(word, "ANIMTEX"))
				...
				else if (!strcmp(word, "CHARACTER"))
				{
					if (i < 15)
						...
						deh_warning("Character %d out of range", i);
					DEH_WriteUndoline(word, word2, UNDO_HEADER);
				}
				...
				else if (!strcmp(word, "LEVEL"))
				{
					if (i > 0 && i <= NUMMAPS)
						...
						deh_warning("Level number %d out of range", i);
					DEH_WriteUndoline(word, word2, UNDO_HEADER);
				}
				else if (!strcmp(word, "CUTSCENE"))
				{
					if (i > 0 && i < 129)
						...
						deh_warning("Cutscene number %d out of range", i);
					DEH_WriteUndoline(word, word2, UNDO_HEADER);
				}
				else if (!strcmp(word, "UNLOCKABLE"))
				{
					if (i > 0 && i < 16)
						...
						deh_warning("Unlockable number %d out of range", i);
					DEH_WriteUndoline(word, word2, UNDO_HEADER);
				}
				else if (!strcmp(word, "FRAME"))
				{
					if (i < NUMSTATES && i >= 0)
						...
						deh_warning("Frame %d doesn't exist", i);
					DEH_WriteUndoline(word, word2, UNDO_HEADER);
				}
/*				else if (!strcmp(word, "POINTER"))
				...
				else if (!strcmp(word, "SOUND"))
				{
					if (i < NUMSFX && i >= 0)
						...
						deh_warning("Sound %d doesn't exist", i);
					DEH_WriteUndoline(word, word2, UNDO_HEADER);
				}
/*				else if (!strcmp(word, "SPRITE"))
				{
				...
				else if (!strcmp(word, "HUDITEM"))
				{
					if (i >= 0 && i < NUMHUDITEMS)
						...
						deh_warning("HUD item number %d out of range", i);
					DEH_WriteUndoline(word, word2, UNDO_HEADER);
				}
...
#endif
	CONS_Printf("Unloading WAD SOC edits\n");
	while (curundo)
	{
		data = curundo->undata;
