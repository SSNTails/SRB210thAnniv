...
static void P_NetArchivePlayers(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		WRITEULONG(save_p, players[i].tagcount);
...
static void P_NetUnArchivePlayers(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		players[i].tagcount = READULONG(save_p); // Number of tags player has avoided
...
static boolean P_UnArchiveSPGame(void)
{
	...
	G_DeferedInitNew(gameskill, G_BuildMapName(gamemap), savedata.skin, false, true);
...
static boolean P_NetUnArchiveMisc(void)
{
	...
	if (!P_SetupLevel(gamemap, gameskill, true))
		return false;
