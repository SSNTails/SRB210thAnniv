...
{
	size_t      i, j = po->numLines-1 ;
	boolean  oddNodes = false;

	for (i = 0; i < po->numLines; i++)
	{
		if ((po->vertices[i]->y < y && po->vertices[j]->y >= y) ||  (po->vertices[j]->y < y && po->vertices[i]->y >= y))
		{
			if (po->vertices[i]->x + (y-po->vertices[i]->y) / (po->vertices[j]->y - po->vertices[i]->y) * (po->vertices[j]->x - po->vertices[i]->x) < x)
				oddNodes = !oddNodes;
		}
		j = i;
	}
...
newseg:
	...
	po->isBad = true;
	CONS_Printf("polyobject %d is not closed", po->id);
}
...
static void Polyobj_findExplicit(polyobj_t *po)
{
	...
	for (i = 0; i < numsegs; ++i)
	{
		...
		Polyobj_GetInfo(segs[i].linedef->tag, &polyID, &mirrorID, NULL);

		if (polyID == po->id && mirrorID > 0)
		{
			...
			segitems[numSegItems].num = mirrorID;
			++numSegItems;
		}
	}
	...
	{
		po->isBad = true;
		CONS_Printf("polyobject %d is empty", po->id);
		return;
	}
...
static void Polyobj_spawnPolyObj(int num, mobj_t *spawnSpot, int id)
{
	...
	for (i = 0; i < numsegs; ++i)
	{
		...
		Polyobj_GetInfo(seg->linedef->tag, &polyID, &mirrorID, NULL);
		...
		if (polyID == po->id)
		{
			...
			if (seg->linedef->flags & ML_PASSUSE)
				po->flags |= POF_RENDERPLANES;

			if (seg->linedef->flags & ML_ALLTRIGGER)
	...
	if (po->segCount == 0)
	{
		unsigned short exparg;
		Polyobj_findExplicit(po);
		// if an error occurred above, quit processing this object
		if (po->isBad)
			return;

		Polyobj_GetInfo(po->segs[0]->linedef->tag, NULL, NULL, &exparg);
		po->mirror = exparg;
		if (po->mirror == po->id) // do not allow a self-reference
			po->mirror = -1;
	...
	if (Polyobj_GetForNum(po->id))
	{
		...
		po->isBad = true;
		CONS_Printf("polyobject id conflict: %d", id);
	}
	else
	{
...
static void Polyobj_moveToSpawnSpot(mapthing_t *anchor)
{
	...
	{
		CONS_Printf("bad polyobject %d for anchor point", anchor->angle);
		return;
	}
	...
	if (po->attached)
	{
		CONS_Printf("polyobj %d has more than one anchor", po->id);
		return;
	}
...
static void Polyobj_pushThing(polyobj_t *po, line_t *line, mobj_t *mo)
{
	...
	{
		if (!P_CheckPosition(mo, mo->x, mo->y, mo->x + momx, mo->y + momy))
...
int EV_DoPolyObjRotate(polyrotdata_t *prdata)
{
	...
	while ((po = Polyobj_GetMirror(po)))
	{
		...
		th->speed =
			(prdata->speed * diracc * prdata->direction
				* (ANG45/45)) >> 3;
		diracc = (diracc > 0) ? -1 : 1;
...
int EV_DoPolyObjMove(polymovedata_t *pmdata)
{
	...
	while ((po = Polyobj_GetMirror(po)))
	{
		...
		th->angle = ((pmdata->angle) + angadd) >> ANGLETOFINESHIFT;
...
static void Polyobj_doSwingDoor(polyobj_t *po, polydoordata_t *doordata)
{
	...
	while ((po = Polyobj_GetMirror(po)))
	{
		...
		th->speed = (doordata->speed * (ANG45/45) * diracc) >> 3;
		diracc = (diracc > 0) ? -1 : 1;
...
int EV_DoPolyObjFlag(line_t *pfdata)
{
	...
	while ((po = Polyobj_GetMirror(po)))
	{
		...
		th->polyObjNum = po->id;
		th->distance   = 0;
		th->speed      = P_AproxDistance(pfdata->dx, pfdata->dy);
		th->angle      = R_PointToAngle2(pfdata->v1->x, pfdata->v1->y, pfdata->v2->x, pfdata->v2->y);
		th->momx       = sides[pfdata->sidenum[0]].textureoffset>>FRACBITS;
