...
#if defined (__APPLE_CC__) || defined(__BIG_ENDIAN__) // Software code don't work in big endian systems
...
static inline VOID DisableAero(VOID)
{
	...
	if (pfnDwmIsCompositionEnabled && SUCCEEDED(pfnDwmIsCompositionEnabled(&pfnDwmEnableCompositiond)))
		I_OutputMsg("Got the result of DwmIsCompositionEnabled, %i", pfnDwmEnableCompositiond);
	else
		return;
...
void I_StartupGraphics(void)
{
	...
	else if (M_CheckParm("-opengl") || rendermode == render_opengl)
	{
