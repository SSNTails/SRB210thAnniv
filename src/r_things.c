...
static int sprpatchheight; // flippy
...
void R_DrawMaskedColumn(column_t *column)
{
	...
	for (; column->topdelta != 0xff ;)
	{
		if (dc_vflip)
		{
			oldval = column->topdelta; //flippy
			column->topdelta = (byte)(sprpatchheight-column->length-column->topdelta); //flippy
		}
...
static void R_DrawVisSprite(vissprite_t *vis, int x1, int x2)
{
	...
	{
		dc_vflip = 1;
		sprpatchheight = patch->height; // flippy

		if (colfunc == basecolfunc)
			colfunc = flipbasecolfunc;
		else if (colfunc == transcolfunc)
			colfunc = fliptranscolfunc;
		else if (colfunc == fuzzcolfunc)
			colfunc = flipfuzzcolfunc;
		else if (colfunc == transtransfunc)
			colfunc = fliptranstransfunc;
	}
...
static void R_ProjectSprite(mobj_t *thing)
{
	...
	vis->gy = thing->y;
	if (thing->scale > 400)
		vis->gz = gzt - FIXEDSCALE(spritecachedinfo[lump].height,400);
	else if (thing->scale != 100)
		vis->gz = gzt - FIXEDSCALE(spritecachedinfo[lump].height,thing->scale);
	else if (thing->flags & MF_HIRES)
		vis->gz = gzt - spritecachedinfo[lump].height/2;
	else
...
void SetPlayerSkinByNum(int playernum, int skinnum)
{
	if (skinnum >= 0 && skinnum < numskins) // Make sure it exists!
	{
		...
		if (atoi(skins[skinnum].waterskip) == 1)
			players[playernum].charflags |= SF_WATERSKIP;
