...
//
#ifdef DEALIGNED
#define WRITEBYTE(p,b)      do {    byte *p_tmp = (   byte *)p; const    byte tv = (   byte)(b); memcpy(p, &tv, sizeof(   byte)); p_tmp++; p = (void *)p_tmp; } while (0)
#define WRITECHAR(p,b)      do {    char *p_tmp = (   char *)p; const    char tv = (   char)(b); memcpy(p, &tv, sizeof(   char)); p_tmp++; p = (void *)p_tmp; } while (0)
#define WRITESHORT(p,b)     do {   short *p_tmp = (  short *)p; const   short tv = (  short)(b); memcpy(p, &tv, sizeof(  short)); p_tmp++; p = (void *)p_tmp; } while (0)
#define WRITEUSHORT(p,b)    do {  USHORT *p_tmp = ( USHORT *)p; const  USHORT tv = ( USHORT)(b); memcpy(p, &tv, sizeof( USHORT)); p_tmp++; p = (void *)p_tmp; } while (0)
#define WRITELONG(p,b)      do {    long *p_tmp = (   long *)p; const    long tv = (   long)(b); memcpy(p, &tv, sizeof(   long)); p_tmp++; p = (void *)p_tmp; } while (0)
#define WRITEULONG(p,b)     do {   ULONG *p_tmp = (  ULONG *)p; const   ULONG tv = (  ULONG)(b); memcpy(p, &tv, sizeof(  ULONG)); p_tmp++; p = (void *)p_tmp; } while (0)
#define WRITEFIXED(p,b)     do { fixed_t *p_tmp = (fixed_t *)p; const fixed_t tv = (fixed_t)(b); memcpy(p, &tv, sizeof(fixed_t)); p_tmp++; p = (void *)p_tmp; } while (0)
#define WRITEANGLE(p,b)     do { angle_t *p_tmp = (angle_t *)p; const angle_t tv = (angle_t)(b); memcpy(p, &tv, sizeof(angle_t)); p_tmp++; p = (void *)p_tmp; } while (0)
#else
...
#ifdef __GNUC__
#ifdef DEALIGNED
#define READBYTE(p)         ({    byte *p_tmp = (   byte *)p;    byte b; memcpy(&b, p, sizeof(   byte)); p_tmp++; p = (void *)p_tmp; b; })
#define READCHAR(p)         ({    char *p_tmp = (   char *)p;    char b; memcpy(&b, p, sizeof(   char)); p_tmp++; p = (void *)p_tmp; b; })
#define READSHORT(p)        ({   short *p_tmp = (  short *)p;   short b; memcpy(&b, p, sizeof(  short)); p_tmp++; p = (void *)p_tmp; b; })
#define READUSHORT(p)       ({  USHORT *p_tmp = ( USHORT *)p;  USHORT b; memcpy(&b, p, sizeof( USHORT)); p_tmp++; p = (void *)p_tmp; b; })
#define READLONG(p)         ({    long *p_tmp = (   long *)p;    long b; memcpy(&b, p, sizeof(   long)); p_tmp++; p = (void *)p_tmp; b; })
#define READULONG(p)        ({   ULONG *p_tmp = (  ULONG *)p;   ULONG b; memcpy(&b, p, sizeof(  ULONG)); p_tmp++; p = (void *)p_tmp; b; })
