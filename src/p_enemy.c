...
void A_SetMoveCount(mobj_t *actor);
...
void A_Look(mobj_t *actor)
{
	...
	if (!P_LookForPlayers(actor, locvar1, false , 0))
		return;
...
void A_MonitorPop(mobj_t *actor)
{
	...
	remains = P_SpawnMobj(actor->x, actor->y, actor->eflags & MF_VERTICALFLIP ? actor->z + actor->height - mobjinfo[actor->info->speed].height : actor->z, actor->info->speed);
...
void A_AttractChase(mobj_t *actor)
{
	...
	if (actor->target && actor->target->player
		&& !actor->target->player->powers[pw_ringshield] && actor->info->reactiontime && actor->type != (mobjtype_t)actor->info->reactiontime)
	{
		mobj_t *newring;
	...
	P_SetTarget(&actor->tracer, actor->target);

	if (!actor->target)
	{
		P_SetTarget(&actor->target, P_SetTarget(&actor->tracer, NULL));
		return;
	}
...
void A_GrenadeRing(mobj_t *actor)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		if (players[i].mo == actor->target)
			continue; // Don't blow up at your owner.

		if (P_AproxDistance(P_AproxDistance(players[i].mo->x - actor->x, players[i].mo->y - actor->y),
			players[i].mo->z - actor->z) > explodedist)
			continue;
...
//
// Whether a mace ball or a chain link of it,
// rotate around your axis target.
//
// var1 = unused
// var2 = unused
//
...
//	boolean changecolor = (actor->target && actor->target->player);
	...
	for (d = 0; d < 16; d++)
		P_SpawnParaloop(actor->x, actor->y, actor->z + actor->height, actor->info->painchance, 16, MT_NIGHTSPARKLE, d*(ANGLE_45/2), true);
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		if (teleported[i])
		{
				...
				&& players[i].mo && players[i].mo->health > 0 && !players[i].exiting)
			{
				...
				P_CheckPosition(players[i].mo, players[i].mo->x, players[i].mo->y, players[i].mo->x, players[i].mo->y);
			}
		}
	}
...
// Function: A_SetMoveCount
//
// Description: Sets the object's 'movecount' value.
//
// var1 = value
// var2 = unused
//
...
// Note: upper bound should not be higher than lower + 256
//
// var1 = lower bound
// var2 = upper bound
//
...
void A_BossJetFume(mobj_t *actor)
{
	...
	else if (locvar1 == 1) // Boss 3 propeller
	{
		...
		filler = P_SpawnMobj(jetx, jety, actor->z + 8*FRACUNIT, MT_PROPELLER);
		P_SetTarget(&filler->target, actor);
...
//
// Description: var1 is the remote object. var2 is the state reference for calling the action called on var1.
// var1 becomes the actor's target, the action (var2) is called on var1. actor's target is restored
// var1 = remote object
