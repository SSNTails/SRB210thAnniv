...
static void V_DrawTranslucentMappedPatch(int x, int y, int scrn, patch_t *patch, const byte *colormap)
{
	...
	if (scrn & V_8020TRANS)
		...
		translevel = ((tr_trans90)<<FF_TRANSSHIFT) - 0x10000 + transtables;
	else
...
void V_DrawSmallScaledPatch(int x, int y, int scrn, patch_t *patch)
{
	...
	if (skippixels)
	{
		...
		rowfrac = FixedDiv(FRACUNIT, (dupy)<<(FRACBITS-1));
	}
	else
	{
		colfrac = FixedDiv(FRACUNIT, dupx<<FRACBITS);
...
static void V_BlitScaledPic(int x1, int y1, int scrn, pic_t * pic);
...
void V_DrawFadeScreen(void)
{
	...
	{
		buf = (long *)(screens[0] + vid.width*y);
		for (x = 0; x < w; x++)
		{
			if (buf+ x > (const long *)deststop)
				return;
